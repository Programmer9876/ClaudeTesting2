...
def build_pool(best_path: Optional[str], archive: List[str], depth: int, beam: int, expand: int,
    ...
    pool: List[str] = []
    if best_path:
        pool += [f"{base},model={best_path}",
                 f"{base},model={best_path},eps=0.03,temp=0.3",
                 f"{base},model={best_path},eps=0.08,temp=0.7",
                 f"{base},model={best_path},temp=1.0"]
    else:
    ...
    for old in archive[-2:]:
        if old != best_path:
            pool.append(f"{base},model={old},eps=0.03,temp=0.3")
    return pool
...
def train(args: argparse.Namespace) -> Dict[str, object]:
    ...
    for it in range(len(history), len(history) + args.iters):
        t_it = time.time()
        pool = build_pool(best_path, archive, args.depth, args.beam, args.expand, rng)
        ...
        hist = net.fit(Xtr, ytr, epochs=args.epochs, batch_size=args.batch_size, lr=args.lr,
                       weight_decay=args.weight_decay, X_val=Xva, y_val=yva)
        ...
        cand_spec = f"{base},model={cand_path}"
        best_spec = f"{base},model={best_path}" if best_path else base
        t_ev = time.time()
