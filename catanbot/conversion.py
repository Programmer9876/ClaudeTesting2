...
* ``C_me = -KAPPA_CONV x R x (c(leaf) - c(root))`` static points.  Anchoring at the root changes no ranking (a
  constant shift of our own static value never reorders softmax leaves) but keeps the correction exactly 0 on
  every leaf where we built nothing, so the hub's fast path returns the C++ values there.

``KAPPA_CONV = 0.12`` points per extra card is static's price of a held card.  Calibration: our typical
post-setup economy (wheat 6.3, ore 5.1, wood / brick 3.4, sheep 2.6 pips) has ``c = 0.16`` cards per roll at 4:1,
13 extra cards over an 80-roll game - the proof logs show ~15 (5 bank trades at 4:1 a game); a generic port then
saves 4.2 cards (measured ~5) and a 2:1 wheat port 3.2 (measured 3.6-3.8), worth 0.4-0.8 points at 0.12 a card -
the range the ports design calibrated for the static 3:1 term (0.76-0.79).  winpaths' ``KAPPA_F = 0.6`` (a future
card spent in a race) would make an early ore/wheat city cost more than its production credit.
...
of a candidate spot to the setup score.
"""
...
_PARAM_NAMES = ("KAPPA_CONV", "REACH_W", "NEED", "CACHE_CAP")
...
class ConversionContext:
    ...
    def __init__(self, root: GameState, me: int, params: Optional[Dict[str, Any]] = None):
        ...
        self.c0 = self.cost(root)
        self.reach0 = self.reach_saving(root) if self.reach_w else 0.0
    ...
    def corrections(self, state: GameState) -> Optional[List[float]]:
        """``[0, ..., -KAPPA_CONV R (c - c_root - REACH_W (reach - reach_root)), ..., 0]`` (our seat only);
        ``None`` when nothing changed since the root."""
        self.stats["evals"] += 1
        d = self.cost(state) - self.c0
        if self.reach_w:
            d -= self.reach_w * (self.reach_saving(state) - self.reach0)
        if d == 0.0:
            return None
        self.stats["nonzero"] += 1
        out = [0.0] * self.n
        out[self.me] = -self.scale * d
        return out
