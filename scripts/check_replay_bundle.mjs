...
// 10. parsers
{
  ...
  // a game present in this bundle (game 5 when there is one; a partial bundle may hold only games 0 and 1)
  const r5 = idx.byId.get(`${t0}-5`) || idx.rows.find(r => r.t === t0);
  const g5 = r5.g;
  const cases = [];
  const ph = h => RC.parseHash(h, idx);
  let x = ph(`#${t0.toLowerCase()}-${g5}`);
  cases.push([x.view === "game" && x.id === `${t0}-${g5}` && x.k === null, `#${t0.toLowerCase()}-${g5} opens ${t0}-${g5}`]);
  x = ph(`#${t0}-${games + 599}`);
  ...
  x = ph(`#${t0}-${g5}.9999`);
  cases.push([x.view === "game" && x.k === r5.acts && x.message === `Game ${t0}-${g5} has ${r5.acts} actions; showing the end`, `#${t0}-${g5}.9999`]);
  x = ph(`#${t0}-${g5}.57`);
  cases.push([x.view === "game" && x.k === Math.min(57, r5.acts) && (57 <= r5.acts ? !x.message : !!x.message), `#${t0}-${g5}.57`]);
  x = ph(`#${t0}`);
